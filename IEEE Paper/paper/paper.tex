...
This paper proposes the adaptation of the FlipIt formulas as presented in \cite{FlipIt} such as to take the delay for virus propagation into account. In the next section we first present the original FlipIt game. Then section [] presents the FlipIt game with virus propagation. Section [] presents some related work. Section [] concludes the paper and presents avenues for further research.
...
To obtain the formula in case of a too long delay, we therefore need to substract this overlapping gain from the above formula.  The overlapping gain is equal to 
~~ \\
...
Finally researchers also have investigated the behaviour of humans playing FlipIt. A. Nochenson and Grossklags \cite{nochenson2013behavioral}  investigate how people really act when given temporal decisions. Reitter et al. \cite{reitter2013risk} observed continuous games, 20-seconds FlipIt game.

\section{Conclusions and Furhter Research}
\label{ch:conclusion}
In this paper we presented an adaptation of the FlipIt game to the situation of virus propagation, such as to take the delay for network infection into account. We discerned two cases. In the case the defender plays faster than the attacker, the attacker simply looses the delay. In the case the attacker plays faster, each time the defender plays in an interval, he will gain the extra time of the delay. The delay is therefore always detrimental to the benefit of the attacker. 
The formula for the second case is a slight overestimation of the defender's benefit in case the defender plays faster than the attacker period extende with the delay. When the defender plays in an interval, he will gain extra time in the next interval, since the attacker has to regain control. However, not the entire delay will be extra time, as the defender may move in again sooner. 
Futher research needs to be performed to calculate the exact overestimation in this case.\\
Also the impact of the delay on nash equilibria and the determination of optimal defender and attacher strategies needs further investigation.
