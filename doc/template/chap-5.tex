...
Cyber security is a part of security that focuses on securing computer controlled devices such as computers and smartphones, as well as public and private computer networks, including the entire Internet. A private network can be an enterprise network that is protected from the public network such as the internet.  The purpose of security is to give certainty that data will not be removed without authorization (Confidentiality) that the data is always accessible (Availability), and that data can not be read or altered by someone who does not have the authorization (Integrity). These are the 3 key attributes of security, also known as the CIA triad. \\
 

In order to keep a system secure, it is important to mitigate the possible threats. A threat refers to anything that can breach the security and that can cause a possible harm. The some of the most common threats are spam, malware, spoofing, phishing and  DDoS attacks. 
The biggest threat are caused by malware. 
This is a piece of malicious program that is designed to penetrate unprotected systems or computers, and getting there to sensitive information. Malware can be divided into different subclasses. These subclasses are not mutually exclusive:
...
A security report of 2014 reveales that 61\% of the attacks on companies are caused by malware. A company can take different measures to defend himself against malware. 
The threats caused by malware can be devided into 3 big groups: known threats (70\%), unknown threats (29\%) and advanced threats (1\%) [\todo{toevoegen referentie naar kaspersky APT report}]. 


The known threats easy to defend against. Unkown also. Then the remaining 1\% are the advanced threats. These threats are also known as APT. APT's are ... 
In this paper we focus on targeted attacks. Targeted attacks are malware designed to attack exclusively an organisation. Some examples of the biggest most rare APTs  to get an understanding of what APTs are capable of and how long they can stay unseen. [\todo{site kaspersky apt}] 

An APT is a persistent targeted attack that tries to penetrate a network, harm and stays unseen for a long period of time. The motive of an APT is mostly cyber espionage, stealing sensitive data, sabotage or other ideological attacks. APT stands for Advanced persistant threat. Advanced stands for the fact that these attacks are well funded and that the attacker itself has a great expertise. These APT are not easy to protect against. Not all APTs are technical advanced. The attacker can also try to exploit existing vulnerabilities and hope that his target has not yet secured himself against these vulnerabilities. Persistent is the fact that the attacker keeps on trying to break in to his victim. He will not give up. The attack can be over various years and different steps. The threats stands for the fact that an APT is ..
