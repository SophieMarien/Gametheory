...
This means that if $\delta_{D} < k_{D}$  the function is negative and if $\delta_{D} > k_{D}$ the function is positive. If $k_{D} < 0$ the function will decrease, and if $k_{D} > 0$ the function will increase.
