...
However, for some of the propagation methods, the actual graph of the network matters. Each propagation method depends on having a right topology. Email worms need a topology that represent a social network, BGP routing worms need a topology on network level. 
Ideally we need a method to calculate the delay that does not assume a specific kind of network topology, but where we can use the actual topology at hand to calculate the delay. \\
...
In this chapter we propose a method to calculate the spread of a worm in a way that the specific topology of the network can be easily integrated. The method will give an approximation of how fast a worm can infect a network. This method is capable of calculating the delay for any network topology. 
...
While matrix calculations may seem very time consuming, in the domain of network analytics efficient algorithms are available capable of calculating network attributes per node in networks of millions of nodes. In this paper, we only provide a proof of concept of how to apply matrices to calculate the delay. Developing an efficient algorithm is considered as a potential avenue for further research. 
...
\textit{The Google matrix is a stochastic matrix that is used to calculate the PageRank of all the web pages on the internet. The dominant eigenvalue of the matrix is the PageRank. The matrix is a graph where the edges denote the links between each page. Using the power method, the PageRank of each page can be calculated iteratively. } --Moet anders verwoord worden. Hoe ?  komt uit wikipedia\\
...
\todo{nog een algemene conclusie schrijven ?}
