...
%There are some difficulties with this scanning technique. First it is difficult to obtain the whole address space from the DNS records. Secondly, the worm has to carry the database, which can slow down the propagation if the database is to big. Last, t
The IP addresses stored in the address table are only hosts with public domain names. This propagation method is used by \todo{aanvullen}.
...
The methods of propagation can be divided into different model types. We are only interested in models that are based on SI model type (Susceptible-Infected), because in our model every node that is infected will stay infected. In table \ref{tree} the taxonomy of the propagation methods are given and the type of model. \\

For more information about other models the reader is refered to the following papers with other most common propagation models: \cite{wang2014modeling}, \cite{OnWorms2005survey}, \cite{xiang2009propagation} and \cite{serazzi2004computer}.
...
AAWP stands for Analytic Active Worm Propagation. The AAWP model is a discrete time propagation model. This model is used for worms that propagate through uniform scanning. When uniform scanning is used, each IP address 
...
\subsubsection{Email worms simulation model}
