...
nodes. One of the players, the defender, will also try to defend his network.
...
The action of the defender consist of defending his network. The network consists of multiple nodes. The defender flips all the nodes of the network in every move he plays. The action of the defender can be extended to the action where the defender only flips a subset of the nodes of the network. This extention has already been investigated in the context of placing anti-virus systems in a graph. This problem is known to be NP-hard. [\todo{referentie}].
The defender has only one action. Sending different viruses and worms to a computer network. Every time the attacker flips the resource, he does this by sending a new virus to the system. The attacker does not send the same virus again, because if the computers are patched, the virus cannot get in any more. Sending a virus in this context is equal to flipping a node in the network. The node can be a targeted node or a random node. If the attacker has knowledge about the graph, the attacker can choose to target the node that can infect all the nodes in the shortest time possible.
...
\todo{cost of the attacker ook toevoegen?}
...
\subsubsection{Motivation Application: Wormlike ATP}
