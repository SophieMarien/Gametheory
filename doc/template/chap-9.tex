...
By defining the resource as a network with multiple nodes it is possible to extend the actions of the defender and the attacker to actions depending on which node is being flipped. These actions will be explained in the following subsection.
...
denoted as an propagation delay variable \textit{d} (called 'delay' for short in the remainder of this paper). If we want to measure how long it takes for the virus to
...
The cost of the attacker depend on the complexity of the vulnerabilities exploited and how difficult it was to program the malware.
...
The paper from Laszka \citep{MitigationNonTargeted}, which was similar to this one, also concluded that a periodic strategy is the dominant strategy against all other non-adaptive strategies.
