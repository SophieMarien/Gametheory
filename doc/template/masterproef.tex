...
G_{D}(t) + G_{A}(t) = t
\end{equation}
De gemiddelde winst van speler \textit{i} is als volgt:
\begin{equation}
\gamma_{i}(t) = G_{i}(t) / t.
\end{equation}
...
\todo{dit nog aanpassen}
De formalisering begint bij het model van de niet-adaptieve continue basis FlipIt spel waarbij spelers een periodieke strategie gebruiken met willekeurige fase. De motivatie voor de keuze komt er door de veronderstelling dat in de meeste bedrijven, de verdediging strategie is om het netwerk periodiek verdedigen. Deze periodieke komt overeen met een defensiestrategie. Een periodieke aanvaller strategie wordt ook uitgegaan In staat om de resultaten te vergelijken met de periodieke strategie van het spel Flipit in \cite{Flipit}. %, As esta ook overeen met een real-life gemeenschappelijke strategie. % Nog verder Eventueel je motiveren
